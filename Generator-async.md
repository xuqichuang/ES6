...
1. "传值调用"（call by value）：比如C 语言：

先计算 `x+5`再把之歌值传入 `f`函数

2. “传名调用”（call by name）：比如Haskell 语言：

先把`x+5`传入 `f`函数，用到的时候在求值
...
函数`f`的第一个参数是一个复杂的表达式，但是函数体内根本没用到。对这个参数求值，实际上是不必要的。因此，有一些计算机学家倾向于"传名调用"，即只在执行时求值。
...
`yield`命令用于将程序的执行权移出 Generator 函数，那么就需要一种方法，将执行权再交还给 Generator 函数。Thunk 函数就派上用场了，因为它可以在回调函数里，将执行权交还给 Generator 函数。
