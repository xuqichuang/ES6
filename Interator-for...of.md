...
函数`readLinesSync`接受一个文件对象作为参数，返回一个遍历器对象，其中除了`next`方法，还部署了`return`方法。下面的两种情况，都会触发执行`return`方法。
...
```javascript
var engines = new Set(["Gecko", "Trident", "Webkit", "Webkit"]);
for (let [key, value] of engines.entries()) {
...
类似数组的对象包括好几类：字符串、DOM NodeList 对象、`arguments`对象。
