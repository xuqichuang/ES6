...
前六种是：`undefined`、`null`、`boolean`、`number`、`string`、`object`
...
创建 Symbol 的时候，可以添加一个描述。[ES2019](https://github.com/tc39/proposal-Symbol-description) 提供了一个实例属性`description`，直接返回 Symbol 的描述。
...
`Symbol.keyFor()`方法返回一个已登记的 Symbol 类型值的`key`。`Symbol`则无返回`key`
